(() => {
  ...
  window.RageRecurrenciasSaveV251 = true;
  ...
  const clientePorId = id => listaClientes().find(cliente => Number(cliente?.id) === Number(id));
  const entrenadorPorId = id => listaEntrenadores().find(entrenador => Number(entrenador?.id) === Number(id));
  ...
  function fechaFinFormulario(modal, startDate) {
  ...
  function fechasCandidatas(modal) {
    ...
    const end = fechaFinFormulario(modal, start);
  ...
  function existeDuplicada(cliente, date, hour) {
  ...
  function conflictoEntrenador({ trainerId, date, hour, duration }) {
    ...
    for (const client of listaClientes()) {
      for (const session of sesionesCliente(client)) {
        if (!session || session.fecha !== date) continue;
  ...
  function guardarLocalmente(clientId, sessionIds) {
    const serialized = JSON.stringify(listaClientes());
    localStorage.setItem('clientes', serialized);

    const stored = JSON.parse(localStorage.getItem('clientes') || '[]');
    const client = stored.find(item => Number(item?.id) === Number(clientId));
    if (!client || !Array.isArray(client.clases)) throw new Error('No se ha encontrado el cliente tras guardar.');

    const storedIds = new Set(client.clases.map(session => String(session?.id || '')));
    const missing = sessionIds.filter(id => !storedIds.has(String(id)));
  ...
  function crearSerie(modal, button) {
    ...
    try {
      ...
      const endDate = fechaFinFormulario(modal, startDate);
      ...
      dates.forEach(date => {
        if (existeDuplicada(cliente, date, hour)) {
          duplicates += 1;
          return;
        }

        if (conflictoEntrenador({ trainerId, date, hour, duration })) {
          conflicts += 1;
          return;
        }
      ...
      try {
        guardarLocalmente(cliente.id, createdSessions.map(session => session.id));
      } catch (storageError) {
      ...
      alert(`No se han podido guardar las sesiones recurrentes. Detalle: ${detail}`);
    } finally {
  ...
  function prepararModal(modal) {
    ...
    if (!saveButton || saveButton.dataset.rageSaveV251 === '1') return;
    saveButton.dataset.rageSaveV251 = '1';
  ...
  function activarDesdeEvento(event) {
    ...
    crearSerie(modal, button);
    return true;
  }

  document.addEventListener('pointerup', event => {
    if (event.pointerType === 'mouse') return;
    if (activarDesdeEvento(event)) suppressSyntheticClickUntil = Date.now() + 1000;
  }, true);
  ...
  window.RageRecurrenciasSaveVersion = VERSION;
})();
