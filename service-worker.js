const CACHE_NAME = "rage-training-v2.4.21";
const APP_SHELL = [
  "./",
  "./index.html",
  "./manifest.json",
  ...
  "./cliente-sesion-v2.4.19.js?v=2.4.21"
];
...
self.addEventListener("fetch",event=>{const request=event.request;if(request.method!=="GET")return;const url=new URL(request.url);if(url.origin!==self.location.origin)return;let networkRequest=request;if(url.pathname.endsWith('/cliente-sesion-v2.4.19.js')&&url.searchParams.get('v')!=='2.4.21'){const fresh=new URL(url.href);fresh.searchParams.set('v','2.4.21');networkRequest=new Request(fresh.href,{method:'GET',headers:request.headers,mode:request.mode,credentials:request.credentials,redirect:request.redirect});}event.respondWith(fetch(networkRequest,{cache:"no-store"}).then(response=>{if(response&&response.status===200){const copy=response.clone();caches.open(CACHE_NAME).then(cache=>cache.put(request,copy));}return response;}).catch(async()=>{const cached=await caches.match(request);if(cached)return cached;if(request.mode==="navigate")return caches.match("./index.html");return Response.error();}));});